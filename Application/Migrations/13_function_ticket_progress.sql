...
BEGIN
	SELECT 
		CEIL(SUM(ts2.percent_progress) / (SELECT SUM(ts3.percent_progress) FROM tbl_ticket_state ts3 JOIN tbl_project_ticket_state pts ON pts.ticket_type = ts3.ticket_type AND pts.ticket_state = ts3.ticket_state WHERE ts3.ticket_type = ts1.ticket_type) * 100)
	INTO
		progress
	FROM
...
BEGIN
  SELECT SUM(percent_progress) / COUNT(id) INTO progress FROM (
	SELECT t.id, ticket_state_progress(t.project_id, t.ticket_type, t.ticket_state) AS percent_progress FROM tbl_ticket t WHERE t.id = param_ticket_id
	UNION 
