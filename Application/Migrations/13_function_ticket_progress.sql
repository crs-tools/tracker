...
BEGIN
	SELECT 
		CEIL(SUM(ts2.percent_progress) / (SELECT SUM(ts3.percent_progress) FROM tbl_ticket_state ts3 JOIN tbl_project_ticket_state pts ON pts.ticket_type = ts3.ticket_type AND pts.ticket_state = ts3.ticket_state WHERE ts3.ticket_type = ts1.ticket_type AND pts.project_id = param_project_id) * 100)
	INTO
		progress
	FROM
