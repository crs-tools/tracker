...
CREATE OR REPLACE VIEW view_serviceable_tickets AS 
	...
	LEFT JOIN
		tbl_ticket_state depender_ticket_state ON
			depender_ticket_state.ticket_type = t.ticket_type AND
			depender_ticket_state.ticket_state = t.ticket_state
	LEFT JOIN
	...
	WHERE
		...
		COALESCE(dependee_ticket_state.sort >= configured_trigger_state.sort, true) AND
		COALESCE(dependee_ticket_state.sort > depender_ticket_state.sort, true)
	ORDER BY
